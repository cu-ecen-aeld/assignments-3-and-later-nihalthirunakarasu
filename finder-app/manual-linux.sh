...
if [ ! -e ${OUTDIR}/linux-stable/arch/${ARCH}/boot/Image ]; then
    ...
    # Making the kernel config files
    make -j4 ARCH=${ARCH} CROSS_COMPILE=${CROSS_COMPILE} all
    # Building the modules
    make ARCH=${ARCH} CROSS_COMPILE=${CROSS_COMPILE} modules
    # Building for device tree
...
# Creating the root
mkdir ${OUTDIR}/rootfs
# cd'ing into the root directory
cd ${OUTDIR}/rootfs
# Creating the basic directories as per Filesystem Hierarchy Standard
mkdir -p bin dev etc home lib lib64 proc sbin sys tmp usr var
# Making /usr/bin, /usr/lib and /usr/sbin
